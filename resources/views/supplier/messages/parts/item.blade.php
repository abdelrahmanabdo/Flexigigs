...
?>
    @if($message->conflect==1)
        ...
        <div class="item msgbox">
            <div class="item-trigger collapsed" data-toggle="collapse" data-parent="#messagesList" data-target="#message-{{$message->id}}" aria-expanded="false">
                <div class="item-info-collapsed">
                    @include('supplier.messages.parts.userdata')
            ...
            <div id="message-{{$message->id}}" class="item-content collapse message-collapse" role="tabpanel">
                <div class="scrollable" data-messageid="{{$message->id}}" data-to="{{Auth::user()->id}}" data-page="0"  data-order_id="{{$message->order_id}}">
                    <?php   $submessages = Flexihelp::get_messages_between($message->id_from,$message->id_to,5,0,$message->order_id);?>
                    @include('supplier.messages.parts.subitem')
                </div>
                <div class="mt-3 d-flex justify-content-end">
                    <a href="#" class="btn btn-primary sendmessage" data-id_to="0" data-order_id="{{$message->order_id}}" data-admin_id="{{$message->admin_id}}" data-conflect="{{$message->conflect}}" data-id_from="{{Auth::user()->id}}">@lang('general.button_reply')</a>
                </div>
            </div>
        </div>
        @else
        <div class="item msgbox">
            ...
            <div id="message-{{$message->id}}" class="item-content collapse message-collapse" role="tabpanel">
                <div class="scrollable" data-messageid="{{$message->id}}" data-to="{{Auth::user()->id}}" data-order_id="{{$message->order_id}}" data-page="0">
                    <?php $submessages = Flexihelp::get_messages_between($message->id_from,$message->id_to,5,0,$message->order_id); ?>
                    @include('supplier.messages.parts.subitem')
                </div>
                <div class="mt-3 d-flex justify-content-end">
    ...
    <div class="item msgbox ">
        ...
        <div id="message-{{$message->id}}" class="item-content collapse message-collapse" role="tabpanel">
            <div class="scrollable" data-messageid="{{$message->id}}" data-to="{{$user_id}}" data-page="0"  data-order_id="{{$message->order_id}}">
                <?php $submessages = Flexihelp::get_messages_between($message->id_from,$message->id_to,5,0,$message->order_id); ?>
                @include('supplier.messages.parts.subitem')
            </div>
            <div class="mt-3 d-flex justify-content-end">
