<div class="filter" style="position: relative;">
    ...
    <form accept-charset="utf-8" onreset="window.location.replace('{{url()->current()}}')">
        ...
        <label class="form-group has-float-label mt-5 mb-0">
            <input type="text" name="from" value="{{@$_GET['from']}}" placeholder="@lang('general.filter_deadline_from')" class="form-control border-0">
			<span for="searchFilter">@lang('messages.from')</span>
			<i class="fas fa-search filter-search-input-icon"></i>
		</label>
		<hr>
        <label class="form-group has-float-label mt-5 mb-0">
            <input type="text" name="to" value="{{@$_GET['to']}}" placeholder="@lang('general.filter_deadline_to')" class="form-control border-0">
			<span for="searchFilter">@lang('messages.to')</span>
			<i class="fas fa-search filter-search-input-icon"></i>			
		</label>
		<hr>
