<input type="hidden" name="customer_id" class="customer_id-{{$assignuserdata->id}}" value="{{$assignuserdata->id}}">
                    ...
                    success: function (message) {
                        ...
                        if (users) {
                            for (var i = users.length - 1; i >= 0; i--) {
                                    content = content+"<li><button type='button' onclick=\"$('.customer_id-{{$assignuserdata->id}}').val("+users[i].id+");hiddenSearch();$('.assignToSupplier-{{$assignuserdata->id}}>.userassign').val('"+users[i].username+"')\">"+users[i].username+"</button></li>";
                            }
