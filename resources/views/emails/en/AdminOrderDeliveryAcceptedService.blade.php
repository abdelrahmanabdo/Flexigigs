...
Service: {{$order->request->name}}</a><br>
Order ID:  <a href="{{route('admin_orders',['order_id'=>$order->id])}}">#{{$order->id}} - {{$order->request->name}}</a><br>
...
الخدمة: {{$order->request->name}}</a><br>
رقم تعريف أمر العمل: <a href="{{route('admin_orders',['order_id'=>$order->id])}}"> #{{$order->id}} - {{$order->request->name}}</a><br>
